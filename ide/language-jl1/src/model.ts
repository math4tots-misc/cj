 ...
 *  - source root path (i.e. path ending in '/src/main/jl1/' or '/src/main/jl1-js/')
...
export function parseUnixPath(unixpath: string): [string, string, string] | null {
    ...
    unixpath = unixpath.substring(0, unixpath.length - '.jl1'.length);
        ...
        while (stack.length > 0) {
            ...
            for (const [name, type] of await this.fs.readDirectory(uri)) {
                ...
                if (type === vscode.FileType.Directory) {
                    stack.push(child);
                } else if (type === vscode.FileType.File && name.endsWith(".jl1")) {
